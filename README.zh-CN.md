...
| 类型 | 开发者是否直接使用 | 职责 |
| --- | --- | --- |
| Pulse preview 仓库 | 是 | 所有人可看的文档、manifest/checksum 和编译后的 preview 制品，用于通用评估。 |
| 客户服务制品 | 是 | 私有客户可运行 Conversation Agent Service 包、workspace、setup/check/doctor、客户服务端服务示例、Web 验证页。 |
| Developer Assistant 仓库 | 是 | 面向 Codex / Cursor / Claude Code 等 AI 编程软件的安装引导、AGENTS.md、skills/plugin、调优流程和排障流程。 |
| 内部产品源码仓库 | 否 | 闭源研发、打包、发布、Live E2E、stable promotion 和制品生成。 |

整体分工是：外部用户通过 `pulse-conversation-agent` 评估服务，AI 编程工具通过本仓库辅助安装/调优/排障，内部工程从源码仓构建和验证，私有客户拿到不可变 `.tgz` 制品在自己的环境部署。
...
flowchart TD
  ...
  B --> C["Pulse preview 制品<br/>pulse-conversation-agent-gateway-v0.1.0-preview.2.tgz"]
  B --> C2["客户私有服务制品<br/>customer-service-package-*.tgz"]
  B --> D["Developer Assistant 仓库<br/>conversation-agent-dev-assistant"]

  C --> E["本地服务包目录<br/>pulse-conversation-agent-gateway-v0.1.0-preview.2/"]
  C2 --> E2["客户本地服务包目录<br/>customer-service-package/"]
  ...
  S --> R
  R --> T["Level 3<br/>云端真实 ZEGO Live E2E 验收"]
  T --> U["release acceptance / stable promotion"]
```
...
按里面的步骤，从 https://github.com/Cogit-oergo-sum/pulse-conversation-agent/releases/tag/v0.1.0-preview.2 安装 Conversation Agent Service。
...
完成后告诉我达到 Level 2.5 还是仍停留在 Level 2，并声明这不是云端 release acceptance。
```
...
| Level 2.5 | 本地 Cloudflare Tunnel 真实 RTC/ZEGO callback smoke | Tunnel URL、入房、麦克风发布、AgentInstance、ASR、LLM callback、TTS、字幕、mode/status/perf；不能替代发布验收。 |

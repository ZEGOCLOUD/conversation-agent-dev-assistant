...
flowchart TD
  ...
  B --> C["Pulse preview 制品<br/>pulse-conversation-agent-gateway-v0.1.0-preview.15.tgz"]
  B --> D["Developer Assistant 仓库<br/>conversation-agent-dev-assistant"]

  C --> E["客户本地服务包目录<br/>pulse-conversation-agent-gateway-v0.1.0-preview.15/"]
...
按里面的步骤从 https://github.com/ZEGOCLOUD/pulse-conversation-agent/releases/tag/v0.1.0-preview.15
安装 Pulse Conversation Agent 0.1.0-preview.15。
...
从 GitHub Release 下载新 artifact，校验 sha256，保留我已有的客户项目目录，
停止旧服务，切换 runtime 服务包目录，运行 check/status/doctor，
...
| --- | --- | --- |
| `pulse-conversation-agent-gateway-v0.1.0-preview.x/` | GitHub Release 下载的不可变 runtime 包。 | 下载、校验、解压，并切换到新目录。 |
...
切换 `current` 后，运行 `check`、`status` 和 `doctor`。如果新版本提示缺少配置，请有意识地更新客户项目，并且不要把密钥写进聊天。
